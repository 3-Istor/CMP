...
def _avg_usage(app_id: int, resource: str, ref: date) -> float:
    ...
    for offset in range(_ANALYSIS_WINDOW_DAYS):
        total += usage.simulated_usage_pct(app_id, resource, ref - timedelta(days=offset))
    return total / _ANALYSIS_WINDOW_DAYS


def _peak_usage(app_id: int, resource: str, ref: date) -> float:
    return max(
        usage.simulated_usage_pct(app_id, resource, ref - timedelta(days=offset))
        for offset in range(_ANALYSIS_WINDOW_DAYS)
    )
...
def recommendations_for_app(app: AppSpec, ref: date | None = None) -> list[Recommendation]:
    ...
    if q.replicas > 1 and load < _REPLICA_LOAD_THRESHOLD:
        ...
        if target < q.replicas:
            ...
            recs.append(Recommendation(
                ...
                app_id=app.id, app_name=app.name, project_id=app.project_id,
                rec_type="replicas",
    ...
    if cpu_peak < _RIGHTSIZE_PEAK_THRESHOLD or ram_peak < _RIGHTSIZE_PEAK_THRESHOLD:
        new_cpu = q.cpu_cores
        new_ram = q.ram_gb
        ...
        if new_cpu < q.cpu_cores or new_ram < q.ram_gb:
            ...
            recs.append(Recommendation(
                ...
                monthly_saving_eur=_saving(app, new_q, ref),
                confidence=_confidence(max(cpu_peak, ram_peak), _RIGHTSIZE_PEAK_THRESHOLD),
                effort="medium",
                patch={"resources": {"limits": {
                    "cpu": new_cpu, "memory": f"{new_ram}Gi"}}},
            ))
    ...
    if storage_avg < _STORAGE_USE_THRESHOLD and q.storage_gb > 5:
        ...
        if target < q.storage_gb:
            ...
            recs.append(Recommendation(
                ...
                monthly_saving_eur=_saving(app, new_q, ref),
                confidence=_confidence(storage_avg, _STORAGE_USE_THRESHOLD),
                effort="high",  # PV resize is disruptive
                patch=None,
            ))

    return [r for r in recs if r.monthly_saving_eur > 0 or r.rec_type == "inactivity"]


def recommendations_for_apps(apps: list[AppSpec], ref: date | None = None) -> list[Recommendation]:
