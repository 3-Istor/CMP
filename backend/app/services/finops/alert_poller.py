...
async def check_budgets() -> None:
    ...
    try:
        ...
        for budget in budgets:
            ...
            level = _level_for(consumed_pct, budget.threshold_warn, budget.threshold_critical)
            ...
            if state is None:
                state = BudgetAlertState(project_name=budget.project_name, last_level=0)
                db.add(state)
                db.commit()
            ...
            if level > state.last_level:
                ...
                logger.info(
                    ...
                    budget.project_name, consumed_pct, level,
                )
...
async def check_spikes() -> None:
    ...
    try:
        ...
        for deployment in deployments:
            ...
            db.commit()
            logger.info("FinOps spike alert for app %s (%s)", deployment.name, spiking)
    except Exception as exc:
